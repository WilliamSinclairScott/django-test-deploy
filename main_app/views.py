...
# Updated DogList and DogDetail views below
# Operating /dogs/
class DogList(generics.ListCreateAPIView):
  ...
  def get_queryset(self):
      # This ensures we only return dogs belonging to the logged-in user
      user = self.request.user
      return Dog.objects.filter(user=user)

  def perform_create(self, serializer):
      # This associates the newly created dog with the logged-in user
      serializer.save(user=self.request.user)

# Operating /dogs/<id:num>
...
class RemoveToyFromDog(APIView):
  def patch(self, request, dog_id, toy_id):
    dog = Dog.objects.get(id=dog_id)
    toy = Toy.objects.get(id=toy_id)
    dog.toys.remove(toy)
    return Response({'message': f'Toy {toy.name} removed to Dog {dog.name}'})
